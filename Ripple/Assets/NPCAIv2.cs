...
public class NPCAIv2 : MonoBehaviour {
	...
	private bool validpath(List<Vector3> nodes) {
		if (nodes [0] == null) {
			return false;
		}
		Vector3 LastNode = transform.position;
		foreach (Vector3 node in nodes) {
			Vector2 CastDirection = (Vector2)(node - LastNode);
			RaycastHit2D Wallcheck = Physics2D.Raycast (LastNode, CastDirection, CastDirection.magnitude, 1 << LayerMask.NameToLayer ("ActualWalls"));
			if (Wallcheck.collider != null) {
				return false;
			}
			LastNode = node;
	...
	void FixedUpdate() {
		if (lighton) 
		{
			...
			{
				Vector3 spawnDistance = transform.position - initialpos;
				if (spawnDistance.magnitude > 0.05f) {
					List<Vector3> nodepos = new List<Vector3> ();
					//Debug.Log ("Exited Detection Range");
					pathfind (initialpos, nodepos);	
					if (!validpath (nodepos)) {
						Debug.Log ("Bad pathfinding");
					}
					followpath (nodepos);
				} else {
